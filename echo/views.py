# -*- coding: UTF-8 -*-
from .models import Node,Line,Employee,Device,Task,Process,Upload,OggInfo, CompareCount
...
def index(request):
    ...
    inspection_number = OggInfo.objects.count()
    #获取已结单的数量,用于计算任务完成率
...
def lists(request, table):
    ...
    if table == 'device':
        ...
        sub_title = '设备信息'
    if table == 'smogginfo':
        raw_data = OggInfo.objects.filter(ogginfo_type__contains='SANMA')
        list_template = 'ogginfo_list.html'
        sub_title = '三码OGG信息'
    if table == 'ttogginfo':
        raw_data = OggInfo.objects.filter(ogginfo_type__contains='TOTAL')
        list_template = 'ogginfo_list.html'
        sub_title = '统一库OGG信息'
    if table == 'dcogginfo':
        raw_data = OggInfo.objects.filter(ogginfo_type__contains='DC-CENTER')
        list_template = 'ogginfo_list.html'
        sub_title = '数据中心OGG信息'
    if table == 'comparecount':
        raw_data = CompareCount.objects.all()
        list_template = 'comparecount_list.html'
        sub_title = '数据比对'
    ...
    data_list, page_range, count, page_nums = pagination(request, data, 10)
    #增加依据传入的URL指定page_tile
    page_title = '基础资料' if table in ['node','device'] else '巡检预警'
