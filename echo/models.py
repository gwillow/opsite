...
#建立ogg巡检信息表
class OggInfo(models.Model):
    ogginfo_prov = models.CharField(verbose_name='省简称',max_length=16)
    ogginfo_type = models.CharField(verbose_name='OGG类型',max_length=64)
    ogginfo_host = models.GenericIPAddressField(verbose_name='主机IP')
    ogginfo_status = models.TextField(verbose_name='进程状态',max_length=512)
    ogginfo_detail = models.TextField(verbose_name='进程明细',max_length=1024)
    ogginfo_diskspace = models.TextField(verbose_name='磁盘空间',max_length=1024)
    ogginfo_tablespace = models.TextField(verbose_name='库表空间',max_length=1024)
    ogginfo_delay = models.CharField(verbose_name='是否延迟',max_length=16)
    ogginfo_suspend = models.CharField(verbose_name='是否挂起',max_length=16)
    ogginfo_createtime = models.DateField(verbose_name='巡检时间')

    def __unicode__(self):
        return self.ogginfo_prov
#建立巡检信息数据比对表
class CompareCount(models.Model):
    comparecount_prov = models.CharField(verbose_name='省简称',max_length=16)
    comparecount_type = models.CharField(verbose_name='类型',max_length=64)
    comparecount_tab = models.CharField(verbose_name='表名',max_length=64)
    comparecount_src = models.TextField(verbose_name='源端数量',max_length=512)
    comparecount_dst = models.TextField(verbose_name='目标数量',max_length=1024)
    comparecount_diff = models.TextField(verbose_name='差异数量',max_length=1024)
    comparecount_date = models.DateField(verbose_name='巡检时间')

    def __unicode__(self):
        return self.comparecount_prov
#建立任务表
